...
/**
 * Created by song on 2018/3/30 0030.
 * 自定义下拉刷新的头布局  以后如果想自定义可以根据这个改改
 *
 */

public class CustomListViewHeader extends RelativeLayout implements RefreshHeader {
    ...
    public void setStyle(int style) {
        this.style = style;
        if (style == 2){
            mTitleText.setTextColor(0xffBABABA);
        }else {
    ...
    public CustomListViewHeader(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        this.initView(context, attrs);
    }

    @RequiresApi(Build.VERSION_CODES.LOLLIPOP)
    public CustomListViewHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void initView(Context context, AttributeSet attrs) {
        ...
        mImageView.setImageResource(R.mipmap.shauxin1_00030);
        mImageView.setVisibility(INVISIBLE);
        LayoutParams param = new LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT,LinearLayout.LayoutParams.WRAP_CONTENT);
        param.addRule(RelativeLayout.CENTER_HORIZONTAL);//addRule参数对应RelativeLayout XML布局的属性
        param.topMargin = getResources().getDimensionPixelSize(R.dimen.dp_30);
        mImageView1.setLayoutParams(param);

        RelativeLayout relativeLayout = new RelativeLayout(context);
        relativeLayout.addView(mImageView);
        relativeLayout.addView(mImageView1);

        mTitleText = new TextView(context);
        mTitleText.setText(REFRESH_HEADER_PULLDOWN);

//        mTitleText.setTextColor(0xff999999);
//        mTitleText.setBackgroundColor(getResources().getColor(R.color.white));
        mTitleText.setGravity(Gravity.CENTER_HORIZONTAL);
        mTitleText.setPadding(0,0,0, density.dip2px(5));

        LinearLayout.LayoutParams lpUpdateText = new LinearLayout.LayoutParams(WRAP_CONTENT, WRAP_CONTENT);
        layout.addView(relativeLayout, lpUpdateText);
//        layout.addView(mImageView, lpUpdateText);

        LinearLayout.LayoutParams lpHeaderText = new LinearLayout.LayoutParams(MATCH_PARENT, WRAP_CONTENT);
        ...
        if (ta.hasValue(com.scwang.smartrefresh.layout.R.styleable.ClassicsHeader_srlTextSizeTitle)) {
            mTitleText.setTextSize(TypedValue.COMPLEX_UNIT_PX, ta.getDimensionPixelSize(com.scwang.smartrefresh.layout.R.styleable.ClassicsHeader_srlTextSizeTitle, DensityUtil.dp2px(12)));
        } else {
        ...
        if (getPaddingTop() == 0) {
            if (getPaddingBottom() == 0) {
                setPadding(getPaddingLeft(), mPaddingTop = density.dip2px(1), getPaddingRight(), mPaddingBottom = density.dip2px(1));
            } else {
                setPadding(getPaddingLeft(), mPaddingTop = density.dip2px(1), getPaddingRight(), mPaddingBottom = getPaddingBottom());
            }
        } else {
            if (getPaddingBottom() == 0) {
                setPadding(getPaddingLeft(), mPaddingTop = getPaddingTop(), getPaddingRight(), mPaddingBottom = density.dip2px(1));
            } else {
    ...
    public void onPullingDown(float percent, int offset, int headHeight, int extendHeight) {
        ...
        if (bitmap == null){
            setStyle(style);
            if (style == 2){
                bitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.shauxin_00000);
            }else {
                bitmap = BitmapFactory.decodeResource(getResources(), R.mipmap.shauxin1_00000);
            }
        }
        mImageView1.setVisibility(VISIBLE);
        mImageView1.setImageBitmap(zoomImg(bitmap, percent));
    }
...
//                restoreRefreshLayoutBackground();
            case PullDownToRefresh:
                ...
                mImageView.setVisibility(INVISIBLE);
                mImageView1.setVisibility(VISIBLE);
                mImageView1.setImageResource(R.mipmap.shauxin1_00000);
                break;
            case Refreshing:
                mTitleText.setText(REFRESH_HEADER_REFRESHING);
                mProgressView.setVisibility(GONE);
                ...
                mImageView1.setVisibility(GONE);
                mImageView.setVisibility(VISIBLE);
                switch (style){
                    case 2:
                        mImageView.setImageResource(R.drawable.animation_loading_frame);
                        break;
                    default:
                        mImageView.setImageResource(R.drawable.animation_loading_frame2);
                        break;
                }
...
//    }
    ...
    public CustomListViewHeader setProgressBitmap(Bitmap bitmap) {
        mProgressDrawable = null;
        mProgressView.setImageBitmap(bitmap);
        return this;
    }

    public CustomListViewHeader setProgressDrawable(Drawable drawable) {
        mProgressDrawable = null;
        mProgressView.setImageDrawable(drawable);
        return this;
    }

    public CustomListViewHeader setProgressResource(@DrawableRes int resId) {
        mProgressDrawable = null;
        mProgressView.setImageResource(resId);
        return this;
    }

    public CustomListViewHeader setArrowBitmap(Bitmap bitmap) {
        mArrowDrawable = null;
        mArrowView.setImageBitmap(bitmap);
        return this;
    }

    public CustomListViewHeader setArrowDrawable(Drawable drawable) {
        mArrowDrawable = null;
        mArrowView.setImageDrawable(drawable);
        return this;
    }

    public CustomListViewHeader setArrowResource(@DrawableRes int resId) {
        mArrowDrawable = null;
        mArrowView.setImageResource(resId);
        return this;
    }

    public CustomListViewHeader setLastUpdateTime(Date time) {
        mLastTime = time;
    ...
    public CustomListViewHeader setTimeFormat(DateFormat format) {
        mFormat = format;
        return this;
    }

    public CustomListViewHeader setSpinnerStyle(SpinnerStyle style) {
        this.mSpinnerStyle = style;
        return this;
    }

    public CustomListViewHeader setPrimaryColor(@ColorInt int primaryColor) {
    ...
    public CustomListViewHeader setAccentColor(@ColorInt int accentColor) {
    ...
    public CustomListViewHeader setPrimaryColorId(@ColorRes int colorId) {
        setPrimaryColor(ContextCompat.getColor(getContext(), colorId));
        return this;
    }

    public CustomListViewHeader setAccentColorId(@ColorRes int colorId) {
        setAccentColor(ContextCompat.getColor(getContext(), colorId));
        return this;
    }

    public CustomListViewHeader setFinishDuration(int delay) {
        mFinishDuration = delay;
        return this;
    }

    public CustomListViewHeader setEnableLastTime(boolean enable) {
    ...
    public CustomListViewHeader setTextSizeTitle(float size) {
    ...
    public CustomListViewHeader setTextSizeTitle(int unit, float size) {
    ...
    public CustomListViewHeader setTextSizeTime(float size) {
        if (mRefreshKernel != null) {
            mRefreshKernel.requestRemeasureHeightForHeader();
        }
        return this;
    ...
    public CustomListViewHeader setTextSizeTime(int unit, float size) {
        if (mRefreshKernel != null) {
            mRefreshKernel.requestRemeasureHeightForHeader();
        }
        return this;
    ...
    public CustomListViewHeader setDrawableMarginRight(float dp) {
        return setDrawableMarginRightPx(DensityUtil.dp2px(dp));
    }

    public CustomListViewHeader setDrawableMarginRightPx(int px) {
    ...
    public CustomListViewHeader setDrawableSize(float dp) {
        return setDrawableSizePx(DensityUtil.dp2px(dp));
    }

    public CustomListViewHeader setDrawableSizePx(int px) {
    ...
    public CustomListViewHeader setDrawableArrowSize(float dp) {
        return setDrawableArrowSizePx(DensityUtil.dp2px(dp));
    }

    public CustomListViewHeader setDrawableArrowSizePx(int px) {
        ViewGroup.LayoutParams lpArrow = mArrowView.getLayoutParams();
        lpArrow.width = px;
        lpArrow.height = px;
        mArrowView.setLayoutParams(lpArrow);
    ...
    public CustomListViewHeader setDrawableProgressSize(float dp) {
        return setDrawableProgressSizePx(DensityUtil.dp2px(dp));
    }

    public CustomListViewHeader setDrawableProgressSizePx(int px) {
        ViewGroup.LayoutParams lpProgress = mProgressView.getLayoutParams();
        lpProgress.width = px;
